...
/************************wangxing added************************/
void OpenFlowSwitchNetDevice::SetSimuTime(Time simuTime)
{
...
int OpenFlowSwitchNetDevice::ReceiveHello (const void *msg)
{
  ...
  probe_control_info* pci = (probe_control_info*)msg;
  int error = 0;
  switch (pci->type) {
    case PROBE_MONITOR: {
    ...
    default:
      NS_ASSERT_MSG(false, "can't receive this probe control message!");
      error = -1;
  }
  return error;
}

void OpenFlowSwitchNetDevice::SendProbe(void)
{
  NS_LOG_FUNCTION(this);
...
OpenFlowSwitchNetDevice::ForwardControlInput (const void *msg, size_t length)
{
  ...
  switch (oh->type)
    ...
    /*************wangxing added******************/
    case OFPT_HELLO:
      error = length < sizeof(ofp_header) ? -EFAULT : ReceiveHello (msg);
      break;
    /*************wangxing added******************/
