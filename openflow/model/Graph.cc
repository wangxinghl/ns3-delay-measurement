...
void Graph::_import_from_file( const std::string& input_file_name )
{
	...
	double edge_weight;
	// int vertex_id = 0;		// wangxing delete
	...
	{
		std::cerr << "The number of nodes in the graph is "<<  m_vtVertices.size() << " instead of " << m_nVertexNum << std::endl;
		exit(1);
	}
...
BaseVertex* Graph::get_vertex( int node_id )
{
	...
	}else
	{
		...
		{
			// int vertex_id = m_vtVertices.size();		// wangxing delete
